...
class SICLState(Enum):
    IDLE = auto(); OBSERVE = auto(); ASSESS = auto(); PLAN = auto()
    GATE = auto(); ACT = auto(); REVIEW = auto(); UPDATE = auto()
...
class DeltaLog:
    ...
    chi: float = 0.5
    omega: float = 0.5
    comp: float = 0.5          # composite index (혼란도)
    e_est: float = 0.0         # estimated risk (위험도)
